...
export function LoginForm({ cinematic = false }: { cinematic?: boolean }) {
  ...
  if (cinematic) {
    return (
      <form action={action} className="w-full max-w-md space-y-3" dir="rtl">
        <label className="block">
          <span className="mb-2 block text-sm font-medium text-white/80">رمز دخول مدير النظام</span>
          <input
            className="liquid-glass w-full rounded-full px-5 py-3 text-right text-sm text-white outline-none placeholder:text-white/35 focus:ring-2 focus:ring-white/25"
            name="password"
            type="password"
        ...
        <button className="inline-flex w-full items-center justify-center rounded-full bg-white px-6 py-3 text-sm font-medium text-black transition-colors hover:bg-gray-200" type="submit">
