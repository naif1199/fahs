...
import { ActiveDonutChart, BarsChart, DonutChart, InteractiveAreaChart, RadarDotsChart } from "@/components/charts";
...
export default async function InspectorPerformancePage({ searchParams }: { searchParams: Promise<PerformanceFilters> }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-6">
      <ChartCard title="مقارنة أداء الفاحصين" icon={BarChart3} className="xl:col-span-4"><BarsChart data={context.charts.overall} /></ChartCard>
      <ChartCard title="توزيع حالات المهام" icon={FileText} className="xl:col-span-2"><DonutChart data={context.charts.taskStatus} legend /></ChartCard>
      <ChartCard title="أعلى جودة توثيق" icon={LineChart} className="xl:col-span-3"><RadarDotsChart data={context.charts.documentation} /></ChartCard>
      <ChartCard title="أكثر الفاحصين إنجازًا" icon={Users} className="xl:col-span-3"><ActiveDonutChart data={context.charts.productivity} /></ChartCard>
      <ChartCard title="متوسط زمن إنجاز التقرير" icon={LineChart} className="xl:col-span-4"><InteractiveAreaChart data={context.charts.reportHours} /></ChartCard>
